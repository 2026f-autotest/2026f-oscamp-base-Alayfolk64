...
| 配置 | 值 |
| --- | --- |
| 阶段 | 基础阶段 — Rust 进阶 & OS 入门 |
| 模板 | [2026f-autotest/2026f-oscamp-base](https://github.com/2026f-autotest/2026f-oscamp-base) |
| 课程编号 | 2074 |
| 总分 | 100 |
...
学员点击课程 README 的领取链接，选课程 **2074** 并提交 Issue。Actions 从 `issue.user.login` 读取账号，复制本模板、设置 `STUDENT_GITHUB`、分配仓库写权限并触发配置检查；机器人回复仓库和邀请链接。助教无需收集名单或逐个运行建仓脚本。
...
`.github/scripts/publish.py` 读取本次运行的结果附件，校验课程、仓库、提交、练习清单、权重与学员身份；将当前实际分数保存到 `gh-pages:course-2074.json` 后，调用固定 OpenCamp 成绩上传 API。只有 `result=1` 才视为成功，HTTP 或业务错误会明确失败。Token 只在上传步骤注入，不出现在源码或日志。
