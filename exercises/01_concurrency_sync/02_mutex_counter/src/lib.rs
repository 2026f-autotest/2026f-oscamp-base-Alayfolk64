...
pub fn concurrent_counter(n_threads: usize, count_per_thread: usize) -> usize {
    ...
    let result = *counter.lock().expect("counter lock poisoned");
    result
}
...
pub fn concurrent_collect(n_threads: usize) -> Vec<usize> {
    ...
    for id in 0..n_threads {
        ...
        workers.push(thread::spawn(move || {
            values.lock().expect("collection lock poisoned").push(id);
        }));
    }
    for worker in workers {
        worker.join().expect("collection worker panicked");
    }
    let mut result = values.lock().expect("collection lock poisoned").clone();
